"""Experiment 0 (forward): forward selection on the real UTD19 ecosystem.

Trains one HistGradientBoosting traffic-forecasting model per city (all cities
in `data/utd19_u.csv`), builds matched response matrices Y_fit / Y_eval, and
runs Section 4.2 forward selection. At every iteration we log a rich metrics
snapshot (|S|, E(S), sum U, per-model U, routing sparsity, ...) to JSON.

Run from project root:
    python experiments/experiment_0forward.py
"""
...
OUT_PATH = os.path.join(_project_root, "results", "experiment_0forward.json")
...
def main() -> None:
    W = 76
    print("=" * W)
    print("EXPERIMENT 0 (FORWARD SELECTION) - UTD19 real traffic ecosystem")
    print("=" * W)
    print(f"  gamma   = {GAMMA}")
    ...
    # ------------------------------------------------------------------
    # Load trained-model bundle (or train on first run)
    # ------------------------------------------------------------------
    ...
    # Always include step 0 (S = empty) so the JSON has a full trajectory
    t0 = time.time()
    step_records = []
    ...
    fs_pruner = ForwardSelectionPruner(coverage_fn=coverage_fn, tolerance_gamma=GAMMA)
    result = fs_pruner.run(debug=False)
    total_time = time.time() - t0

    # Post-process pruner history -> rich per-step metrics
